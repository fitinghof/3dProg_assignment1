...
int main() {
	...
	Shape* shapes[]{
		new Triangle(Vector3D(255, 0, 0),
								 Vector3D(128, 64, 240),
								 Vector3D(128 + 256, 64, 240),
								 Vector3D(128 + 128, 512, 200)),
		new Sphere(Vector3D(0,0,0), Vector3D(256,256,256), 128),
		new OBB(Vector3D(0,0,200), Vector3D(128,128,256),Vector3D(0.7071,0.3535,-0.6124),Vector3D(0.7071,-0.3535,0.6124),Vector3D(1,0,1),50,50,50),
		new Plane(Vector3D(0,255,0), Vector3D(0,0,400), Vector3D(0,0,1))
	};
			...
			for (Shape* shape : shapes) {
				...
				if (shape->Intersection(Ray(rayOrigin, cameraDirection), distance)) {
					if (distance < distances[y * width + x]) {
						...
						colour = colour * (1 / (((distance - 150) / 100) + 1));

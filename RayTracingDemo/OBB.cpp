...
OBB::OBB(const Vector3D& colour, const Vector3D& centerPoint,
...
bool OBB::Intersection(const Ray& ray, double& t)
{
	...
	for (int i = 0; i < 3; i++)
	{
		...
		if (!aproxEqual(angleVecDotRayDir, 0, 0.001)) {
			double currentCloseIntersection = (angleVecDotRayToOBB + sizes[i]) / angleVecDotRayDir;
			double currentFarIntersection = (angleVecDotRayToOBB - sizes[i]) / angleVecDotRayDir;
			...
			// If the OBB doesn't intersect our ray or is behind us or return false
			if (farIntersection < closeIntersection || farIntersection < 0) return false;
		} 
		// incase our ray is parallel to the slabs check if it is outside of the slabs
		else if (-angleVecDotRayToOBB - sizes[i] > 0 || -angleVecDotRayToOBB + sizes[i] < 0) return false;
	}
	// set t to the appropriate length
	if (closeIntersection > 0)
		t = closeIntersection;
	else t = farIntersection;
